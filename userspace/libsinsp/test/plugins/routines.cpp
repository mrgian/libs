...
static ss_plugin_rc plugin_parse_event(ss_plugin_t *s, const ss_plugin_event_input *ev, const ss_plugin_event_parse_input* in)
{
    ...
    {
    case 1:
        ps->routine = ps->routine_vtable.subscribe(ps->owner, do_nothing, (ss_plugin_routine_state_t*)&ps->flag);
        break; 
    case 2:
        ps->routine_vtable.unsubscribe(ps->owner, ps->routine);
        break;
    case 3:
        ps->routine = ps->routine_vtable.subscribe(ps->owner, test_routine, (ss_plugin_routine_state_t*)&ps->flag);
        break;
    case 4:
        ps->flag = false;
        break; 
